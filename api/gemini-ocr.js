...
// ── 系統 Prompt：車隊成本分析師（強化版）──
const FLEET_ANALYST_PROMPT = `你是展儀物流（高雄）的車隊成本分析 AI 顧問。你擁有完整的車隊資料庫存取權限。

## 你的能力
1. **車輛查詢**：依車牌查詢任何車輛的維修履歷、費用明細、里程
2. **成本報表**：產出月度/季度/年度成本報表，含趨勢分析
3. **異常偵測**：找出費用異常的車輛，分析原因並給建議
4. **廠商分析**：比較各廠商報價，找出價差
5. **分類統計**：依 7 大類 40 子類分析費用結構
6. **汰換建議**：根據維修頻率與成本評估車輛是否該汰換
7. **跨車比較**：比較同型號車輛的維修成本差異

## 回覆規則
- 一律使用繁體中文
- 用具體數據（車號、金額、日期）支撐每個結論
- 金額加上 $ 和千分位（如 $15,000）
- 如果資料中找不到用戶問的內容，明確說「目前資料中沒有相關紀錄」，不要編造
- 回覆中使用適當 emoji 提升可讀性（📊💰🔴🟢⚠️✅）
- 對比車隊平均值來判斷是否異常
- 若用戶問的不在車隊管理範圍，禮貌引導回正題

## 車隊資料
以下是完整的車隊資料庫（由系統即時提供，請直接使用）：
`;
...
export default async function handler(req, res) {
  ...
  const { mode, imageBase64, mimeType, prompt, vehicleHistory, fleetContext, conversationHistory } = req.body || {};
...
分析要包含：1) 重點摘要 2) 異常提醒 3) 趨勢觀察 4) 改善建議。回覆使用純文字，適合放入 PDF 報表。`;
      ...
      default:
        ...
        // Build enhanced system prompt with fleet data
        systemPrompt = FLEET_ANALYST_PROMPT;
        if (fleetContext) {
          systemPrompt += JSON.stringify(fleetContext, null, 0);
        }
        parts.push({ text: prompt });
        break;
    }

    // Build contents array — support multi-turn conversation for chat mode
    let contents;
    if (mode === 'chat' && Array.isArray(conversationHistory) && conversationHistory.length > 0) {
      // Multi-turn: history + current user message
      contents = [
        ...conversationHistory,
        { role: 'user', parts }
      ];
    ...
    const geminiRes = await fetch(`${GEMINI_URL}?key=${apiKey}`, {
      ...
      body: JSON.stringify({
        ...
        generationConfig: {
          ...
          maxOutputTokens: mode === 'ocr' || mode === 'license' ? 8192 : (mode === 'chat' ? 4096 : (mode === 'report' ? 4096 : 2048)),
